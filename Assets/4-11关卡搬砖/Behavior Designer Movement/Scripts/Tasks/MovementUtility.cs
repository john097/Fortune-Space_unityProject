    ...
    public static class MovementUtility
    {
        ...
        private static Transform WithinSight(Transform transform, Vector3 positionOffset, float fieldOfViewAngle, float viewDistance, Transform targetObject, bool usePhysics2D, out float angle)
        {
            ...
            if (direction.magnitude < viewDistance && angle < fieldOfViewAngle * 0.5f) {
                ...
                if (LineOfSight(transform, positionOffset, targetObject, usePhysics2D, viewDistance) != null) {
        ...
        public static Transform LineOfSight(Transform transform, Vector3 positionOffset, Transform targetObject, bool usePhysics2D, float viewDistance)
        {
#if !(UNITY_4_0 || UNITY_4_0_1 || UNITY_4_1 || UNITY_4_2)
            if (usePhysics2D) {
                RaycastHit2D hit;
...
#endif
                var direction = Vector3.Distance(targetObject.position, transform.position);

                RaycastHit hit;
                if (Physics.Linecast(transform.TransformPoint(positionOffset), targetObject.position, out hit)) {
                    if (hit.transform.Equals(targetObject)||direction<= viewDistance) {
                        return targetObject; // return the target object meaning it is within sight
                    }
                }
#if !(UNITY_4_0 || UNITY_4_0_1 || UNITY_4_1 || UNITY_4_2)
